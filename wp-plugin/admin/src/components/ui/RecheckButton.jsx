...
/**
 * Bouton de re-vérification d'une URL 404 (version WP — pas de siteId).
 ...
 * @param {object|null} initialRecheck - { recheck_status, recheck_final_url, recheck_checked_at } ou null
 */
export default function RecheckButton({ url, initialRecheck }) {
  ...
  async function handleRecheck(e) {
    ...
    setLoading(true)
    try {
      const data = await api.post('/crawler/recheck-url', { url })
      setRecheck({ recheck_status: data.status, recheck_final_url: data.finalUrl, recheck_checked_at: data.checkedAt })
    } catch {
...
function getBadge(recheck, t) {
  ...
  const code = parseInt(recheck.recheck_status, 10)
  if (!code) return null
