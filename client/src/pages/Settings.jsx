...
export default function Settings() {
  ...
  // Charger sitemaps + statut crawl pour tous les sites dès que la liste est disponible
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-prussian-500 rounded-xl border border-prussian-400 p-6 flex flex-col gap-5">
        <div className="flex items-center gap-3 border-b border-prussian-400 pb-4">
          <div className="w-8 h-8 rounded-lg bg-moonstone-400/10 flex items-center justify-center">
            <Icon icon="ph:magnifying-glass" className="text-moonstone-400 text-base" />
          </div>
          <div>
            <h3 className="text-white font-bold text-sm">{t('settings.sectionCrawler')}</h3>
            <p className="text-errorgrey text-xs">{t('settings.sectionCrawlerDesc')}</p>
          </div>
        </div>

        {sites.length === 0 ? (
          <p className="text-errorgrey text-sm italic">{t('settings.noSitesForCrawler')}</p>
        ) : (
          <div className="flex flex-col gap-4">
            {sites.map(site => {
              ...
              return (
                <div key={site.id} className="flex flex-col gap-3 bg-prussian-600 rounded-xl p-4">
                  {/* En-tête du site */}
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {/* Statut crawl */}
                    {isRunning ? (
                      <span className="flex items-center gap-1.5 text-moonstone-400 text-xs font-semibold">
                        <Icon icon="ph:spinner" className="animate-spin text-sm shrink-0" />
                        {crawlStatus.pagesCrawled ?? 0} / {crawlStatus.pagesFound ?? '?'} {t('settings.crawlPages')}
                      </span>
                  ...
                  {sitemaps.length > 0 && (
                    <div className="flex flex-col gap-1">
                      {sitemaps.map(sm => (
                        <div key={sm.id} className="flex items-center justify-between gap-2 bg-prussian-500 rounded-lg px-3 py-2">
                          <div className="flex items-center gap-2 min-w-0">
                            <Icon icon="ph:map-trifold" className="text-moonstone-400 text-sm shrink-0" />
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-prussian-700 border border-prussian-400 rounded-lg px-3 py-2 text-white text-xs placeholder-errorgrey focus:outline-none focus:border-moonstone-400 transition-colors"
                  ...
                  <div className="flex justify-end">
                    {isRunning ? (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-dustyred-400 hover:bg-dustyred-400/10 border border-dustyred-400/40 rounded-lg transition-colors"
                      >
                        <Icon icon="ph:stop-circle" className="text-base" />
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-moonstone-600 hover:bg-moonstone-500 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        <Icon icon="ph:magnifying-glass" className="text-base" />
                        {t('settings.launchCrawl')}
                        {sitemaps.length === 0 && <span className="text-xs font-normal opacity-70">— {t('settings.noSitemapsYet')}</span>}
                      </button>
                    )}
                  </div>
                </div>
